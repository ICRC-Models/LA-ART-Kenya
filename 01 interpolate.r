...
## This function is used to interpolate linearly between observed values to create a smoothed time series.  
## The "breaks" argument contains the sorted year breaks (ie, 1990, 2000) and the "values" argument contains 
## the values corresponding to the breaks.  "step_size" specifies the steps in years (ie, 0.25 = quarter-year 
## step).  Default arguments for the range of the time series are the global variables year_start and year_end.

## The function will return a vector of values.  The length of the vector will be the total number of time 
## steps between (and including) the start and end years.  The values at the breaks will be the values passed 
## in the "values" argument to the function.  The values between the breaks will be linear interpolations 
## between the passed values.  If the minimum and maximum breaks are greater than or less than (respectively) 
## the year_start and year_end, the function will return repeated corresponding values for the years between 
## year_start and the minimum break or year_end and the maximum break.
